...
export const StateDetailsPanel: React.FC<StateDetailsPanelProps> = ({ profile }) => {
  ...
  return (
    <div className="glass-panel p-6 flex flex-col md:flex-row gap-6 items-stretch w-full h-full justify-between">
      
      {/* Left Column: Selected State Risk Profile */}
      <div className="flex-1 flex flex-col justify-between gap-4">
        <div>
          <span className="text-[9px] text-[#8FA6B8] uppercase font-bold tracking-widest block">
            Selected State Risk Profile
          </span>
          <h2 className="text-xl font-extrabold text-textWhite tracking-wide mt-1 uppercase">
            {profile.name}
          </h2>
          ...
          <div className="flex items-baseline gap-2 mt-3.5">
            <span className="text-4xl font-extrabold font-mono" style={{ color: riskColor }}>
              {profile.riskPercentage}%
            </span>
            <div className="flex flex-col">
              <span className="text-[11px] font-bold uppercase tracking-wider" style={{ color: riskColor }}>
          ...
          <div className="flex items-center gap-1.5 text-[9px] text-tealAccent font-semibold mt-3">
          ...
          <div 
            className="mt-4 p-3 rounded-lg border text-[11px] leading-relaxed flex gap-2.5"
            style={{ 
          ...
          >
            ...
            <p className="font-medium text-textWhite leading-relaxed">{profile.alertMessage}</p>
          </div>
        </div>

        <div className="flex justify-between items-center text-[9px] text-textMuted pt-2.5 border-t border-white/5 mt-auto">
      ...
      <div className="hidden md:block w-px bg-white/5 self-stretch my-1" />

      {/* Right Column: Risk Score & Circular Gauge */}
      <div className="flex-1 flex flex-col justify-between gap-4">
        <div>
          <span className="text-[9px] text-[#8FA6B8] uppercase font-bold tracking-widest block">
            Threat Diagnostics
          ...
          <div className="flex flex-row items-center justify-between gap-5 mt-4">
            ...
            <div className="relative flex items-center justify-center w-24 h-24 shrink-0">
              ...
              <div className="absolute flex flex-col items-center justify-center">
                <span className="text-lg font-bold text-textWhite font-mono leading-none">
                  {profile.riskPercentage}%
                </span>
                <span className="text-[7px] text-textMuted uppercase font-bold tracking-wider mt-0.5">
                  Threat Index
                </span>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center text-[10px] pt-2.5 border-t border-white/5 mt-auto">
          <button 
            onClick={() => setShowModal(true)}
            className="text-left text-[11px] font-bold text-tealAccent hover:underline flex items-center gap-1 focus:outline-none"
          >
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bgDark/85 backdrop-blur-sm animate-fadeIn">
          <div className="glass-panel border-white/10 w-full max-w-lg p-6 relative flex flex-col shadow-2xl">
            
            {/* Close Button */}
            <button 
              onClick={() => setShowModal(false)}
              className="absolute top-4 right-4 text-textMuted hover:text-textWhite focus:outline-none"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-3 border-b border-white/10 pb-4 mb-4">
              <div className="p-2 rounded-lg bg-tealAccent/15 border border-tealAccent/20 text-tealAccent">
            ...
            <div className="flex flex-col gap-4 text-xs text-textMuted leading-relaxed">
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                {[
                  ...
                  { name: 'Slope Topography (20%)', desc: 'Physics-based shear stress calculations matching slope angle and local elevations.', icon: Triangle },
                  { name: 'Seismic Telemetry & Historical (20%)', desc: 'Real-time telemetry geophone sensor micro-vibrations and previous slide incidents.', icon: Activity }
                ].map((wt, idx) => (
